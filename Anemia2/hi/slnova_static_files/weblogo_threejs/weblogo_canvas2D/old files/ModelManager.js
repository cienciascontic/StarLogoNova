/*
 * The ModelManager is responsible for handling all 2D drawing assets (loading, storing, organizing, and creating assets).
 */
 ...
 */
ModelManager = function (rotGranularity)
{	
	...
	getModel: function (model, callback) 
	{
    	...
    	if (!(tagRegEx.test(model) && !this.tagToUrl.has(model)))
		{
			...
			if (!this.modelMap.has(model))
				this.modelMap.set(model, new Model(fileUrl, this.rotationGranularity, callback));
			else if (callback)
				callback();
